...
class MonteCarloChargingSimulator:
    ...
    def simulate_day(self, day_type='workday'):
        """模拟一天的充电负荷 (双类终端排队模型)
        ...
        for hour in range(24):
            n_arrivals = arrival_rates[hour]
            if n_arrivals <= 0:
                continue

            actual_arrivals = self.rng.poisson(n_arrivals)
            actual_arrivals = max(0, min(actual_arrivals, n_arrivals * 3))

            for _ in range(actual_arrivals):
                vehicle = self._sample_vehicle()
                arrival_time = hour + self.rng.uniform(0, 1)
                ...
                dur_480 = min(dur_480, 3.0)
                dur_120 = min(dur_120, 3.0)
